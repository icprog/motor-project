...
CIRCULAR_BUF const * CBufInit(CIRCULAR_BUF *pBuf, UINT8 *pStart, UINT32 len, UINT8 optionFlags)
{
	pBuf->flags = optionFlags & (((UINT8)0x01));
	pBuf->len = len;
	pBuf->pStart = pStart;
...
BOOL CBufIsEmpty(CIRCULAR_BUF const *pBuf)
{
	return (BOOL)((pBuf->rdOffset == pBuf->wrOffset) && !(pBuf->flags & (((UINT8)0x80))));
}

BOOL CBufIsFull(CIRCULAR_BUF const *pBuf)
{
	return (pBuf->flags & (((UINT8)0x80)))? ((_Bool)1) : ((_Bool)0);
}
...
UINT32 CBufRead(void *pDest, UINT32 len, CIRCULAR_BUF *pBuf)
{
	...
	if (pBuf->rdOffset >= pBuf->wrOffset)
	{
		...
		pBuf->flags &= ~(((UINT8)0x80));
		pDest = (UINT8 *)pDest + rdLen;	//lint !e826
		len -= rdLen;
	}
	blockLen = ( ((pBuf->wrOffset - pBuf->rdOffset) < (len))? (pBuf->wrOffset - pBuf->rdOffset) : (len) );
...
void CBufSeek(CIRCULAR_BUF *pBuf, INT32 offset)
{
	...
	if (offset > 0)
	{
		...
		max = (INT32)((pBuf->flags & (((UINT8)0x80)))? pBuf->len : (pBuf->wrOffset - pBuf->rdOffset));
		pBuf->rdOffset += (UINT32)( ((offset) < (max))? (offset) : (max) );
		pBuf->flags &= ~(((UINT8)0x80));
	}
	else if (!(pBuf->flags & (((UINT8)0x80))))
	{
		offset = (offset == (-2147483647-1))? 2147483647 : -offset;
		if (pBuf->rdOffset >= pBuf->wrOffset)
		{
...
UINT32 CBufWrite(CIRCULAR_BUF *pBuf, void const *pSrc, UINT32 len)
{
	...
	if ((len > pBuf->len) && !(pBuf->flags & (((UINT8)0x01))))
	{
	...
	while (len > 0)
	{
		if ((pBuf->flags & ((((UINT8)0x80)) | (((UINT8)0x01)))) ==
			  ((((UINT8)0x80)) | (((UINT8)0x01))))
		{
			break;
		}
		...
		blockLen = ( ((len) < (blockLen))? (len) : (blockLen) );
