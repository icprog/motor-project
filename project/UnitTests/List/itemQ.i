...
void ItemQPush(ITEM_Q *pQ, void const *pSrc, u32 len)
{
   ...
   {
      ItemQPop(0, 0xffffffffU, pQ);  //lint !e534
   }

   if (((pQ)->order == ITEM_Q_FIFO))
   {
      // [len][data]
...
static void ItemQPopBytes(void *pDest, u32 len, ITEM_Q *pQ)
{
   ...
   {
      u32 rdLen = ( ((pQ->headIndex) < (len))? (pQ->headIndex) : (len) );
      pQ->headIndex -= rdLen;
      if (pDest)
