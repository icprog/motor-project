...
static FDISK *pFileDisks[FDISK_COUNT] = {0};
...
BOOL FileDevIsMounted(FDISK_NUM disk)
{
	return (((UINT8F)(disk)) < (UINT8F)FDISK_COUNT) && pFileDisks[disk]->isMounted;
}

void FileDevInfo(FDISK_NUM disk, FDISK_STATS *pDest)
{
	if (pDest && (((UINT8F)(disk)) < (UINT8F)FDISK_COUNT))
	{
...
FFILE FileOpen(char const *pName, FOPEN_MODE openMode)
{
	...
	if (!(((UINT8F)(disk)) < (UINT8F)FDISK_COUNT))
	{
	...
	else
	{
		...
		if (pFile)
		{
			...
			if ((((int)(file) < 0) && ((int)(file) >= (int)FERR_CODE_END)))
			{
...
int FileClose(FFILE file)
{
	int retVal;
	FDISK *pDisk;
	{if (!(FileHandleIsValid(file))) return (int)FERR_INVALID;};
...
char * FileGets(FFILE file, char *pStr, int maxLen)
{
	{if (!(FileHandleIsValid(file))) return 0;};
...
int FileStat(char const *pName, FSTAT *pDest)
{
	{if (!((((UINT8F)(FileDiskNumFromStr(pName))) < (UINT8F)FDISK_COUNT))) return (int)FERR_INVALID;};
...
int FileRename(char const *pOld, char const *pNew)
{
	{if (!((((UINT8F)(FileDiskNumFromStr(pOld))) < (UINT8F)FDISK_COUNT))) return (int)FERR_INVALID;};
...
int FileFindFirst(char const *pName, BOOL shouldRecurse, FFIND *pFind)
{
	...
	if (!pFind || !(((UINT8F)(disk)) < (UINT8F)FDISK_COUNT) ||
		 ((pHandle = FileFindHandleOpen(pDisk)) == 0))
	{
		return 0;
	}
	...
	while (pHandle->pFsDirHandle)
	{
		...
		if (retVal == FERR_NONE)
		{
			...
			else if ((pFind->foundType == FFIND_TYPE_DIR) && pHandle->shouldRecurse)
			{
				...
				strncat(pHandle->currentPath, "/", (255));
			}
		}
...
void FileDiskReg(FDISK *pDisk)
{
	if ((((UINT8F)(pDisk->diskNum)) < (UINT8F)FDISK_COUNT) && (pFileDisks[pDisk->diskNum] == 0))
	{
		pFileDisks[pDisk->diskNum] = pDisk;
		pDisk->isMounted = ((_Bool)0);
	}
}

FDISK * FileDiskFromNum(FDISK_NUM diskNum)
{
	return (((UINT8F)(diskNum)) < (UINT8F)FDISK_COUNT)? pFileDisks[diskNum] : 0;
}
...
static FFILE_HANDLE * FileHandleOpen(FDISK *pDisk)
{
	...
	for (UINT16F i = 0; i < (3); i++, pFile++)
	{
		if (pFile->userHandle == 0)
		{
			pFile->userHandle = FileHandleGetNew(pDisk->diskNum, i, ((_Bool)0));
			return pFile;
		}
	}
	return 0;
...
static FFIND_HANDLE * FileFindHandleOpen(FDISK *pDisk)
{
	...
	for (UINT16F i = 0; i < (2); i++, pFind++)
	{
		if (pFind->userHandle == 0)
		{
			pFind->userHandle = FileHandleGetNew(pDisk->diskNum, i, ((_Bool)1));
			return pFind;
		}
	}
	return 0;
...
static BOOL FileHandleIsValid(FFILE file)
{
	...
	if ((((UINT8F)(disk)) < (UINT8F)FDISK_COUNT))
	{
		...
		if (file & ((UINT16)0x8000))
		{
			valid = ((num < (2)) && (pFileDisks[disk]->fileFind[num].userHandle == file));
		}
		else
		{
			valid = ((num < (3)) && (pFileDisks[disk]->fileHandles[num].userHandle == file));
		}
	}
	else
	{
	...
	if (strlen(pHandle->currentPath) > (UINT16F)(FutilsFileName(pHandle->searchName) - pHandle->searchName))
	{
		...
		if (pFolder)
		{
			...
			if (pHandle->pFsDirHandle)
			{
				...
				}while (!res && (strcmp(pFind->fileName, pFolder) != (0)));
			}
		}
	}
	return (pFolder != 0);
...
static int FileRemoveMask(char const *pName)
{
	...
	UINT8F dirLevel = 0;
	FDISK_NUM disk = FileDiskNumFromStr(pName);
	int retVal = FileFindFirst(pName, ((_Bool)0), &find);
	...
	while (retVal == FERR_NONE)
	{
		...
		if (retVal == FERR_NONE)
		{
			...
			if (find.foundType == FFIND_TYPE_DONE)
			{
				...
				{
					retVal = FileFindFirst(pName, ((_Bool)0), &find);
				}
				else if (dirLevel > 1)
				{
					FutilsUpDir(find.fileName);
					strcat(find.fileName, "*");
					retVal = FileFindFirst(find.fileName, ((_Bool)0), &find);
				}
				dirLevel--;
			}
		}
